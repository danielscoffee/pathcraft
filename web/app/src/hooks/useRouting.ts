...
async function solveMode(
	...
	try {
		...
		for (const option of mode.options ?? []) {
			...
			if (value !== undefined) {
				serializedOptions[option.name] =
					option.kind === "time" ? normalizeClockTime(value) : value;
			}
		}
...
export function useRouting(modes: RouteMode[], modeOptions: ModeOptions) {
	...
	const solveAll = useCallback(
		async (
			...
			const activeModes = modesRef.current.filter(
				(mode) => !onlyModeID || mode.id === onlyModeID,
			);
			...
			setStatus({
				text: onlyModeID ? `Routing ${onlyModeID}…` : "Routing all modes…",
				tone: "info",
			});
			...
			setStatus(
				okCount === 0
					? {
							text: `No route found: ${settled[0][1].error ?? "unknown error"}`,
							tone: "err",
						}
	...
	const clearPoint = useCallback((role: "from" | "to") => {
		...
		setStatus({
			text: `Click the map to set ${role === "from" ? "A" : "B"}.`,
			tone: "info",
		});
	}, []);
	...
	const placePoint = useCallback(
		async (lat: number, lon: number) => {
			...
			setStatus({
				text:
					role === "from" ? "Snapping A to the road network…" : "Snapping B…",
				tone: "info",
			});
			try {
