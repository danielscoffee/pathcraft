...
function createStreetLayer(data: GeoJSON.FeatureCollection) {
	return L.geoJSON(data, {
		style: (feature) => {
			const style = highwayStyle(
				(feature?.properties as RouteLayerProps)?.highway,
			);
	...
	useEffect(() => {
		...
		if (!chunks) {
			...
			load
				.then((data) => {
					...
					onStatus({
						text: `Street graph: ${data.features.length} edges shown.`,
						tone: "info",
					});
				})
				.catch((error: Error) => {
					if (!cancelled)
						onStatus({
							text: `Graph load failed: ${error.message}`,
							tone: "err",
						});
				});
		...
		const refresh = () => {
			...
			const desired = new Set(
				mapZoom < chunks.min_render_zoom
					? []
					: visibleChunkIDs(viewportBounds, chunks.zoom),
			);
			...
			if (mapZoom < chunks.min_render_zoom) {
				onStatus({
					text: `Zoom in (≥ ${chunks.min_render_zoom}) to render streets.`,
					tone: "info",
				});
	...
	useEffect(() => {
		...
		const refresh = () => {
			if (map.getZoom() < MIN_ZOOM_NODES) {
				...
				layer.current = null;
				onStatus({
					text: `Zoom in (≥ ${MIN_ZOOM_NODES}) to render nodes.`,
					tone: "info",
				});
				return;
			}
			...
			fetchNodes(bbox, NODES_MAX, NODES_MIN_DEGREE, abort.current.signal)
				...
				.catch((err: Error) => {
					if (err.name !== "AbortError") {
						onStatus({
							text: `Nodes fetch failed: ${err.message}`,
							tone: "err",
						});
					}
				});
	...
	useEffect(() => {
		...
		load
			.then((data) => {
				...
				onStatus({
					text: `Bus stops: ${data.features.length} shown.`,
					tone: "ok",
				});
			})
			.catch((err: Error) => {
				if (!cancelled)
					onStatus({ text: `Bus stops failed: ${err.message}`, tone: "err" });
			});
