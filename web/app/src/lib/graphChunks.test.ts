...
const ordinaryBounds: GraphChunkBounds = { west: -10, south: -10, east: 10, north: 10 }
...
describe('visibleChunkIDs', () => {
  it('returns ordinary viewport tiles in deterministic row-major order', () => {
    ...
    expect(visibleChunkIDs(ordinaryBounds, 2)).toEqual(visibleChunkIDs(ordinaryBounds, 2))
  })

  it('wraps across the antimeridian', () => {
    expect(visibleChunkIDs({ west: 170, south: -10, east: -170, north: 10 }, 2)).toEqual([
      '2/3/1',
      '2/0/1',
      '2/3/2',
      '2/0/2',
  ...
  it('clamps polar bounds to Web Mercator', () => {
    expect(visibleChunkIDs({ west: -10, south: -90, east: 10, north: 90 }, 1)).toEqual([
      '1/0/0',
      '1/1/0',
      '1/0/1',
      '1/1/1',
  ...
  it('caps pathological viewports before allocating an unbounded tile list', () => {
    expect(visibleChunkIDs({ west: -180, south: -90, east: 180, north: 90 }, 20)).toHaveLength(256)
  })
})

describe('graphChunkURL', () => {
  it('substitutes only validated tile integers and an encoded server generation', () => {
    expect(graphChunkURL({ ...config(), generation: 'generation/a' }, '3/4/2')).toBe(
      '/graph/chunks/generation%2Fa/3/4/2',
    )
    expect(() => graphChunkURL(config(), '3/../../secret')).toThrow('invalid graph chunk id')
    expect(() => graphChunkURL({ ...config(), url: 'javascript:{generation}/{z}/{x}/{y}' }, '3/4/2')).toThrow(
    ...
    for (const url of [
      ...
      expect(() => graphChunkURL({ ...config(), url }, '3/4/2')).toThrow('invalid graph chunk config')
    }
  })
})
...
describe('GraphChunkManager', () => {
  it('limits concurrent loads to six and publishes each completed chunk immediately', async () => {
    const pending: Array<ReturnType<typeof deferred<GeoJSON.FeatureCollection>>> = []
  ...
  it('waits for an aborted same-ID flight before loading the new generation', async () => {
    ...
    const viewport = { config: config(), enabled: true, bounds: ordinaryBounds, mapZoom: 3 }
  ...
  it('removes offscreen publications and skips requests below minimum zoom', async () => {
    ...
    manager.update({ config: config(), enabled: true, bounds: ordinaryBounds, mapZoom: 1 })
    expect(loads).toBe(0)

    manager.update({ config: config(), enabled: true, bounds: ordinaryBounds, mapZoom: 3 })
    await settle()
  ...
  it('uses a bounded LRU and retries evicted chunks', async () => {
    ...
    manager.update({ config: config(), enabled: true, bounds: firstViewport, mapZoom: 3 })
    await settle()
    manager.update({ config: config(), enabled: true, bounds: firstViewport, mapZoom: 3 })
    await settle()
