...
export function pruneGraphChunkFailures<T>(failures: Map<string, T>, desired: ReadonlySet<string>) {
  let changed = false
...
interface GraphChunkManagerOptions<T> {
  load: (config: GraphChunkConfig, id: string, signal: AbortSignal) => Promise<T>
...
export function visibleChunkIDs(bounds: GraphChunkBounds, zoom: number): string[] {
  if (
  ...
  if (longitudeSpan >= 360) {
    ...
    xRanges = west <= east ? [[minimumX, maximumX]] : [[minimumX, count - 1], [0, maximumX]]
  }
...
export class GraphChunkManager<T> {
  ...
  constructor(options: GraphChunkManagerOptions<T>) {
    ...
    this.maxConcurrent = Math.max(1, Math.floor(options.maxConcurrent ?? DEFAULT_CHUNK_CONCURRENCY))
    this.maxCache = Math.max(1, Math.floor(options.maxCache ?? DEFAULT_CHUNK_CACHE_SIZE))
  }

  update(viewport: GraphChunkViewport) {
    if (this.disposed) return
    const nextSignature = viewport.config ? configSignature(viewport.config) : ''
    ...
    if (!viewport.enabled || !viewport.config || viewport.mapZoom < viewport.config.min_render_zoom) {
      this.enabled = false
      this.abortAll()
      this.queue = []
      this.desired.clear()
    ...
    this.enabled = true
    const nextDesired = new Set(visibleChunkIDs(viewport.bounds, viewport.config.zoom))
  ...
  private pump() {
    while (this.enabled && this.config && this.flights.size < this.maxConcurrent && this.queue.length > 0) {
      ...
      request
        .then((value) => {
          if (controller.signal.aborted || signature !== this.signature || this.disposed) return
          this.put(id, value)
...
const parseChunkID = (config: GraphChunkConfig, id: string): [number, number, number] => {
  const match = /^(0|[1-9]\d*)\/(0|[1-9]\d*)\/(0|[1-9]\d*)$/.exec(id)
  if (!match) throw new Error(`invalid graph chunk id ${id}`)
  const values = match.slice(1).map(Number)
  const [z, x, y] = values
  if (!values.every(Number.isSafeInteger) || z !== config.zoom || z < 0 || z > 30) {
    throw new Error(`invalid graph chunk id ${id}`)
  }
  const count = 2 ** z
  if (x < 0 || x >= count || y < 0 || y >= count) throw new Error(`invalid graph chunk id ${id}`)
  return [z, x, y]
}

const substitute = (template: string, token: string, value: string) => {
  if (template.split(token).length !== 2) throw new Error('invalid graph chunk URL template')
...
export function graphChunkURL(config: GraphChunkConfig, id: string): string {
  ...
  let url = substitute(config.url, '{generation}', encodeURIComponent(config.generation))
  url = substitute(url, '{z}', String(z))
  url = substitute(url, '{x}', String(x))
  url = substitute(url, '{y}', String(y))
  if (url.includes('{') || url.includes('}')) throw new Error('invalid graph chunk URL template')
  return url
}
