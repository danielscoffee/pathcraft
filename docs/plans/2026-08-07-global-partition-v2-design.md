...
5. Replay ways and resolved nodes in lockstep. Validate every node ID and way
   ...
   and halo tiles.
6. For each `(way, zoom-8 shard)`, append one compact undirected fragment. A
   fragment stores policy/text once plus its shard-relevant endpoint pairs.
   It omits derivable distance, owners, directed edges, source, and target
   tiles.
...
Raw request and resolved-sort inputs may be removed only after their downstream
stage is durably checkpointed. Compact fragment inputs remain until publication
so interrupted or damaged staged shards can be rebuilt.
...
New stage names use a `-v2` suffix. A version-1 work directory completed only
through `select-nodes` is eligible: source identity, build options, counts, and
artifact sizes are revalidated before adding v2 stages. A checkpoint that
...
- Records, strings, segment counts, sort memory, merge fan-in, open files, and
  chunk limits remain bounded.
- Work files are private regular files, atomically replaced and synced at stage
  boundaries. Cancellation never marks an incomplete stage complete.
...
about 79.6 GiB. Sort runs temporarily coexist with one input/output generation,
so stage-wise deletion and high-water reporting are required.
...
- fragment codec corruption and bounds, descriptor LRU reopening, z12 and z8
  seams, antimeridian ownership, restrictions, and cancellation;
