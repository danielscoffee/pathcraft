...
func TestCmdChunksBuildGlobalPublishesAndResumes(t *testing.T) {
	...
	for _, want := range []string{"Stage:", "Generation: " + manifest.Generation, "Shards:", "Chunks:", "Nodes:", "Segments:", "Fragments:", "Fragment bytes:", "Edges:", "Work bytes:", "Store bytes:", "Elapsed:"} {
		if !strings.Contains(output, want) {
			t.Fatalf("output %q lacks %q", output, want)
		}
	}
